...
const initiallyDisplayedCurrencies = ["INR", "USD", "GBP", "JPY"];
let baseCurrency;
...
function currenciesListInputChange(event) {
  ...
  const isNewBaseCurrencyAmount = isNaN(event.target.value) ? 0 : Number(event.target.value);
  if (baseCurrencyAmount !== isNewBaseCurrencyAmount || isNewBaseCurrency) {
    baseCurrencyAmount = isNewBaseCurrencyAmount;
    const baseCurrencyRate = currencies.find(currency => currency.abbreviation === baseCurrency).rate;
    currenciesList.querySelectorAll(".currency").forEach(currencyLI => {
